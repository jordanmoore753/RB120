module Participant
  ...
  def reduce_ace_total
    if over_win_value?
      ace_total = 0
      hand.each { |card| ace_total += 1 if card.value == 'A' }
      ace_total.times do
        @hand_value -= 10
      end
    end
...
class TwentyOne
  ...
  def display_current_cards
    puts "#{player.name}'s cards: #{player.card_values}."
    puts "#{dealer.name}'s first card is: #{dealer.display_first_card}."
  end

  def display_wins
    puts "#{player.name} wins: #{player.wins}."
    puts "#{dealer.name} wins: #{dealer.wins}."
  end
  ...
  def player_turn
    player.hit(deck)
    player.total
    player.reduce_ace_total
  end

  def dealer_turn
    dealer.hit(deck)
    dealer.total
    dealer.reduce_ace_total
  end
